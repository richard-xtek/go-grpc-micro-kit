...
func customCodeToLevel(c codes.Code) zapcore.Level {
	...
	level := grpc_logf.DefaultCodeToLevel(c)
	return level
}
...
func TestZapLoggingSuite(t *testing.T) {
	...
	opts := []grpc_logf.Option{
		grpc_logf.WithLevels(customCodeToLevel),
	}
	b := newBaseZapSuite(t)
	b.InterceptorTestSuite.ServerOpts = []grpc.ServerOption{
		grpc_middleware.WithStreamServerChain(
			grpc_ctxtags.StreamServerInterceptor(grpc_ctxtags.WithFieldExtractor(grpc_ctxtags.CodeGenRequestFieldExtractor)),
			grpc_logf.StreamServerInterceptor(b.log, opts...)),
		grpc_middleware.WithUnaryServerChain(
			grpc_ctxtags.UnaryServerInterceptor(grpc_ctxtags.WithFieldExtractor(grpc_ctxtags.CodeGenRequestFieldExtractor)),
			grpc_logf.UnaryServerInterceptor(b.log, opts...)),
	}
...
func TestZapServerOverrideSuppressedSuite(t *testing.T) {
	...
	opts := []grpc_logf.Option{
		grpc_logf.WithDecider(func(method string, err error) bool {

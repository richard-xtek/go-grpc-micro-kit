...
import (
	...
	"github.com/richard-xtek/go-grpc-micro-kit/grpc-logf/ctx-logf"
...
func (s *loggingPingService) Ping(ctx context.Context, ping *pb_testproto.PingRequest) (*pb_testproto.PingResponse, error) {
	...
	ctx_logf.AddFields(ctx, zap.String("custom_field", "custom_value"))
	ctx_logf.Extract(ctx).For(ctx).Info("some ping")
...
func (s *loggingPingService) PingList(ping *pb_testproto.PingRequest, stream pb_testproto.TestService_PingListServer) error {
	...
	ctx_logf.Extract(stream.Context()).For(stream.Context()).Info("some pinglist")
...
func newBaseZapSuite(t *testing.T) *zapBaseSuite {
	...
	log := log.NewFactory(zap.New(core))
	s := &zapBaseSuite{
		log:         log,
		buffer:      b,
		mutexBuffer: muB,

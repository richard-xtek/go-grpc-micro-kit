...
func customClientCodeToLevel(c codes.Code) zapcore.Level {
	...
	level := grpc_logf.DefaultClientCodeToLevel(c)
	return level
}
...
func TestZapClientSuite(t *testing.T) {
	...
	opts := []grpc_logf.Option{
		grpc_logf.WithLevels(customClientCodeToLevel),
	}
	b := newBaseZapSuite(t)
	b.InterceptorTestSuite.ClientOpts = []grpc.DialOption{
		grpc.WithUnaryInterceptor(grpc_logf.UnaryClientInterceptor(b.log, opts...)),
		grpc.WithStreamInterceptor(grpc_logf.StreamClientInterceptor(b.log, opts...)),
	}
...
func TestZapClientOverrideSuite(t *testing.T) {
	...
	opts := []grpc_logf.Option{
		grpc_logf.WithDurationField(grpc_logf.DurationToDurationField),
	}
	b := newBaseZapSuite(t)
	b.InterceptorTestSuite.ClientOpts = []grpc.DialOption{
		grpc.WithUnaryInterceptor(grpc_logf.UnaryClientInterceptor(b.log, opts...)),

...
// Check level & write logs a msg with fields
func (sl spanLogger) CheckWrite(lvl zapcore.Level, msg string, fields ...zapcore.Field) {
	sl.logToSpan(lvl.String(), msg, fields...)
	if lvl == zapcore.FatalLevel {
		tag.Error.Set(sl.span, true)
	}
	if ce := sl.logger.Check(lvl, msg); ce != nil {
		ce.Write(fields...)
	}
}
...
func (fa *fieldAdapter) AddObject(key string, value zapcore.ObjectMarshaler) error {
	if pom, ok := value.(utils.JsonpbObjectMarshaler); ok {
		bytes, err := pom.MarshalJSON()
		if err != nil {
			return err
		}
		*fa = append(*fa, log.String(key, string(bytes)))
	}
